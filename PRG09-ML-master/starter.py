...
# SUPERVISED LEARNING
def supervisedLearning(logistic=True):
    """logistic or decision trees"""
    ...
    # Print accuracu between Y test and predictions
    print("Trainings accuracy:", accuracy_score(Y, Y_predict))
    ...
    # Z = np.zeros(100) # dit is een gok dat alles 0 is... kan je zelf voorspellen hoeveel procent er goed is? rond de 50%
    ...
    for i in range(len(x)):
        plt.plot(x[i], y[i], 'k.', markersize=8)
        # Plot true data in red
        if (Y[i] == 1): plt.plot(x[i], y[i], 'r.', markersize=8)
    ...
    for i in range(len(x)):
        plt.plot(x[i], y[i], 'k.', markersize=8)
        # Plot true data in red
        if (Y_predict[i] == 1): plt.plot(x[i], y[i], 'r.', markersize=8)
